...
const SearchListing = {
  ...
  async resolve({ request }, {
    ...
    let bookingTypeFilter = {}, minimumAgeFilter = {}, maximumAgeFilter = {}, ageRangeFilter = {}, checkAvailableAges = {};
    //let ageRange = [minimumAge,maximumAge];
    ...
    // if(minimumAge != undefined && Number(minimumAge) >=0 && maximumAge){
    //   checkAvailableAges = await Listing.findAll({
    //     where:{
    //       // id: {
    //       //   $between: [minimumAge, maximumAge]
    //       // },
    //       $or: [
    //         {
    //           $and: [
    ...
    if (minimumAge != undefined && Number(minimumAge) >=0 && maximumAge) {
      ageRangeFilter = {
        ...
        id: {
          $in: [
              sequelize.literal(`SELECT id FROM Listing WHERE ((minAge BETWEEN ${minimumAge} AND ${maximumAge}) OR (maxAge BETWEEN ${minimumAge} AND ${maximumAge}) OR (minAge >= ${minimumAge} AND maxAge <= ${maximumAge}) OR (minAge = 0 AND maxAge = 18))`)

            // sequelize.literal(`SELECT id FROM Listing WHERE ((minAge >= ${minimumAge} AND ${minimumAge} <= maxAge) OR (minAge >= ${maximumAge} AND ${maximumAge} <= maxAge) OR (minAge >= ${minimumAge} AND maxAge <= ${maximumAge}))`)
            // sequelize.literal(`SELECT id FROM Listing WHERE minAge>${minimumAge} AND maxAge<=${maximumAge}`)
          ]
        }
        //  }
        //]   
    ...
    let countLength = listingCount.length;
    return {
      count: countLength,
      results: listingData
    }
