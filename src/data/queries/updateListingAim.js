  ...
  async resolve({ request, response }, {
    ...
    if (request.user || request.user.admin) {
      ...
      if (isListUpdated) {
        ...
        const islearningAimAvailable = await LearningAim.findOne({ where: { listId: id } });
        if (islearningAimAvailable == null) {
          const updateLearningAim = Listing.update({
            isPublished: false,
          },
            {
              where: {
...
export default updateListingAim;
