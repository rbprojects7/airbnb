...
class InboxItem extends Component {
	...
	render() {
		...
		const { title,city, state, country, status, sentBy, read } = this.props;
				...
				<Row>
					...
					>
						<Col xs={7} sm={7} md={5} lg={6} className={cx(s.textTruncate, s.threadBody)}>
							<span className={cx(s.threadSubject, isRead, s.linkColor)}>{content != null ? content : this.label(status, true)}</span>
