...
import writeMentorReviewsQuery from './writeMentorReviews.graphql';
...
class MentorReviewForm extends Component {
  ...
  async handleSubmit(values, dispatch) {
    ...
    const { parentId, childId, isParentEnable, reservationId } = this.props;
    
    //const { writeMentorReviews: { writeMentorReviews } } = this.props;    
...
MentorReviewForm = reduxForm({
  form: 'MentorReviewsForm', // a unique name for this form
})(MentorReviewForm);
...
export default compose(
  ...
  graphql(writeMentorReviewsQuery, {
    options: {
      fetchPolicy: 'network-only'
    }
  }),
