...
// Helper functions
import { formattingTime, checkIn, checkValue } from '../ViewListing/ListingDetails/helper';
...
class CancellationPolicy extends React.Component {
  ...
  render() {
    ...
    if(guestData && hostData && messageData && listData) {
      ...
      let experienceType, cancellationDescription;
      let cancellationPolicy = [];

     if(data && data.listData && data.listData.listingData){
      experienceType =  checkValue(listData.listingData.experienceType, '1');
      cancellationDescription = checkValue(listData.listingData.cancellationDescription, '');
      cancellationPolicy = listData.cancellationPolicy;
    }
      return (
        <div>
          <Grid>
              ...
              <form onSubmit={handleSubmit(submit)}>
                <Col xs={12} sm={7} md={7} lg={7} >
                  <Summary 
                    ...
                    sessionTimes={data.chosenBlockData || undefined}
                    isParentEnable={data.isParentEnable}
                    childParentData={data.childParentData}
                    ...
                    total={data.total}
                    parentId={data.guestId}
                />
                  <Field 
                    className={s.textareaInput}
                    name="message" 
                    component={this.renderFormControlTextArea} 
                    label={formatMessage(messages.guestCancelMessage)} 
                  />
                ...
                }
                {
                  userType === 'guest' && <DetailsForGuest 
                    ...
                    threadId={data.messageData.id}
                    confirmationCode={data.confirmationCode}
                    coverPhoto={listData.coverPhoto}
                    listPhotos={listData.listPhotos}
                    city={listData.city}
                    state={listData.state}
                    country={listData.country}
                    sessionTimes={data.chosenBlockData}
