...
class ListItem extends React.Component {
    ...
    handlePreview(listId) {
        if (listId) {
            history.push(`/experience/${listId}/preview`);
        }
    }

    handleEditListing(listId) {
        if (listId) {
            const { getListingSteps } = this.props;
            getListingSteps(listId);
            history.push(`/become-a-mentor/${listId}/home`);
        }
    }
    ...
    render() {
        ...
        const updatedDate = moment(data.updatedAt).format('Do MMMM YYYY');
        ...
        if (data && data.user) {
            profilePicture = data.user.profile.picture;
            if (profilePicture) {
                profilePhoto = true;
            }
        }
                    ...
                    <li className={s.panelBody} >
                        <Row>
                            ...
                            <Col xs={12} sm={12} md={5} lg={5}>
                                <div className={s.listPhotoCover}>
                                    <div className={s.listPhotoMedia}>
                                        <a target="_blank" href={`/experience/${listId}/preview`}>
                                            <ListCoverPhoto
                            ...
                            <Col xs={12} sm={12} md={7} lg={7} className={s.listDetailContent}>
                                ...
                                <Col xs={12} sm={12} md={11} lg={11} className={cx(s.noPadding, s.spaceTop2)}>
                                    <ProgressBar
                                ...
                                <Col xs={12} sm={10} md={10} lg={10} className={cx(s.noPadding, s.spaceTop2)}>
                                    <a target="_blank" href={`/experience/${listId}/preview`}>
                                ...
                                <a
                                    href={`/experience/${listId}/preview`}
                                    target="_blank"
                                ...
                                {
                                    data && data.isReady && <PublishOption
                                        listId={listId}
                                        isPublished={data.isPublished} learningAimCount={data.learningAimCount} profilePhoto={profilePhoto}
                                    />
                                }
                                {
                                    data && data.isPublished && data.reservationForListing > 0 && <Link
                                        to={`/cancel-blocks/${listId}`}
                                        className={cx('btn btn-default', s.leftMarginDesktop, s.button, s.btnPrimaryBorder, s.spaceTop3, s.smButton)}
                                    >
