...
class BlockListMob extends React.Component {
    static propTypes = {
    ...
    render() {
        const { blocks, loading, currency, listId, isHost, isAuthenticated, account } = this.props;
        const { showInterestData, cookiesValue } = this.state;
        const { current_Status } = this.state;
        const price = this.getPrice(blocks);
        ...
        blocks && blocks.map(async (item, index) => {
            const newSession = {
                firstSession: item.sessionTime[0].date,
                block: item
            };
        ...
        return (
            <div className={`data-sticky bookItFormSection`}>
                <div className={`${s.bookItContainer} ${blockListStyle.blockListWrapper} ${blockListStyle.bookItPanel}`}>
                    {headerText.showCurrency ? <div className={`${blockListStyle.header}`}>
                        <Row style={{ margin: 0, padding: 0, width: '100%' }}>
                            <Col xs={7} sm={7} md={7} lg={7} style={{ paddingLeft: 20 }}>
                                {headerText.text}
                            </Col>
                    ...
                    </div> : <div className={`${s.bookItContentBox} ${blockListStyle.header}`}>
                        <span>{headerText.text}</span>
                    </div>}
              ...
              newBlock && newBlock.map && newBlock.map((item, index) => {
                  ...
                  return (<div>
                      {item && blockDate >= today && <hr /> && (index > 0)}
                      {item && blockDate >= today && <BlockListItem block={item.block} listId={listId} isHost={isHost} key={listId} isAuthenticated={isAuthenticated} isBookingAllowed={isBookingAllowed} />}
                  </div>);
              })
            }
...
export default withStyles(blockListStyle)(connect(mapState, mapDispatch)(BlockListMob));
