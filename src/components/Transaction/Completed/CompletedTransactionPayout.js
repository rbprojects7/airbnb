...
class CompletedTransactionPayout extends Component {
    ...
    render() {
		...
		let sessionDate = data.sessionDate !=null ? moment(data.sessionDate,'Do MMMM YYYY').format('DD-MM-YYYY') : '';

        return (
            <tr>
	          <td className={className}>{date}</td>
			  <td className={className}><FormattedMessage {...messages.transactionPayout} /></td>
			  <td className={className}>{sessionDate}</td>
			  <td className={className}>
			  <p><FormattedMessage {...messages.transferTo} /> {data.payoutEmail} </p>
			  <p><FormattedMessage {...messages.hostCancelMessage5} /> - {data.learnersParticipated} </p>
			  </td>
	          {/* <td className={className} /> */}
	          <td className={className}>
	          	<CurrencyConverter
