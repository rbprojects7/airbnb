    ...
    return (
      <div className={cx(s.progressContainer, "hidden-xs", "hidden-print")}>
        <ul className={cx(s.navList)}>
          ...
          <li className={cx({
            [s.active]: location === "/experience" ||
              location === "/reservation/previous" || location === "/reservation/current"
          })}>
            <Link to={"/experience"} className={s.navItem}>
              <FormattedMessage {...messages.yourListings} />
            </Link>
          </li>

          <li className={cx({ [s.active]: location === "/experiences/current" || location === "/experiences/previous" || location === "/user/reflections"})}>
            <Link to={"/experiences/current"} className={s.navItem} >
