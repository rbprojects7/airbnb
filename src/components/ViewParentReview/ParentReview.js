...
import s from './ParentReview.css';
// Components
...
class ParentReview extends React.Component {

  static propTypes = { 
    data: PropTypes.shape({
      loading: PropTypes.bool,
      getParentReview: PropTypes.arrayOf(PropTypes.shape({
  ...
  render() {
    const {data: { loading, getParentReview }, loadMore } = this.props;
    const { formatMessage } = this.props.intl;
    var showLoadMore = false;
    if (getParentReview && getParentReview.length > 0) {
      showLoadMore = true;
    }
    return (
          ...
          <Panel header={formatMessage(messages.reviews)} className={s.panelNolist} >
            ...
            {
              !loading && (!getParentReview || (getParentReview && 
                getParentReview.length === 0)) && <p>
                <FormattedMessage {...messages.noReview} />
              </p>
            } 
            {
            !loading && getParentReview && <ul 
              className={cx(s.listStyle, s.spaceTop4, s.recommondations)}>
                {
                  getParentReview.map((item, index) => {
                    if(item.yourReviewsCount === getParentReview.length){
                      showLoadMore = false;
                    }
                    if (item.isAdmin) {
                      return <ReviewItem
            ...
            {/* {
              !loading && showLoadMore && <div className={cx(s.space2, s.textCenter)}>
...
export default injectIntl(withStyles(s)(ParentReview));
