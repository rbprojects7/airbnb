    ...
    renderCountryList = ({ input, label, meta: { touched, error }, children, className }) => {
      ...
      return (
          ...
          <label className={s.labelText}><FormattedMessage {...messages.country} /></label>
	    	{touched && error && <span className={s.errorMessage}>{formatMessage(error)}</span>}
	    	<FormGroup className={s.formGroup}>
      			<CountryList input={input} className={className} />
      		</FormGroup>
      	</div>
      );
    }
        ...
        return (
        	<form onSubmit={handleSubmit}>
	            <Panel
	              ...
	              header={formatMessage(messages.addHomeAddress)}
	              footer={
	                <Button
...
export default injectIntl(withStyles(s)(connect(mapState, mapDispatch)(PayoutBillingDetails)));
