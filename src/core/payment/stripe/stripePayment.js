...
import { getHostDetails } from './helpers/getHostDetails';
import { getReservationData } from './helpers/getReservationData';
...
const stripePayment = app => {
    app.post('/stripe-reservation', async function (req, res) {
        ...
        let createCard, createCustomer, source, charge, customerId, hostAccountId, reservationData, guestServiceFee = 0, hostServiceFee = 0, payoutAmount = 0;
        ...
        if (reservationDetails) {
            ...
            hostAccountId = await getHostDetails(reservationDetails.hostId);
            reservationData = await getReservationData(reservationDetails.reservationId);
            guestServiceFee = reservationData.guestServiceFee;
            hostServiceFee = reservationData.hostServiceFee;
            payoutAmount = reservationDetails.amount - guestServiceFee - hostServiceFee;
        } else {
            status = 400;
        ...
        let redirect = '/users/experiences/itinerary/' + reservationDetails.reservationId;     
